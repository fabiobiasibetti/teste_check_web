...
import { CheckSquare, History, Truck, Moon, Sun, Loader2, RefreshCw, LogOut, ChevronRight, ChevronLeft, Cloud } from 'lucide-react';
...
const AppContent = () => {
  ...
  // Sincronização manual (Somente quando o usuário clica)
  const handleManualSync = async () => {
    if (!currentUser || isSyncing) return;
    setIsSyncing(true);
    await loadDataFromSharePoint(currentUser);
    setIsSyncing(false);
  };
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 overflow-hidden">
      <aside className={`bg-white dark:bg-slate-900 border-r dark:border-slate-800 transition-all ${collapsed ? 'w-20' : 'w-64'} p-4 flex flex-col shadow-xl z-50`}>
        <div className="mb-10 flex items-center gap-3">
          <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white font-bold shadow-lg shadow-blue-500/30">V</div>
        ...
        <div className="mt-auto space-y-2 pt-4 border-t dark:border-slate-800">
           <button 
             ...
             disabled={isSyncing}
             className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${isSyncing ? 'text-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'text-slate-500 hover:bg-slate-100'} ${collapsed ? 'justify-center' : ''}`}
             title="Sincronizar com SharePoint"
           >
             <RefreshCw size={20} className={isSyncing ? 'animate-spin' : ''} />
             {!collapsed && <span className="font-bold text-xs uppercase">Sincronizar</span>}
           </button>

           <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-3 w-full flex items-center gap-3 text-slate-500 hover:bg-slate-100 rounded-xl transition-all shadow-sm">
             <div className="w-full flex justify-center items-center gap-3">
                {isDarkMode ? <Sun size={20}/> : <Moon size={20}/>}
                {!collapsed && <span className="font-bold text-xs uppercase">{isDarkMode ? 'Claro' : 'Escuro'}</span>}
             </div>
           </button>

           <button onClick={() => setCollapsed(!collapsed)} className="p-3 w-full flex items-center gap-3 text-slate-500 hover:bg-slate-100 rounded-xl transition-all">
             <div className="w-full flex justify-center items-center gap-3">
                {collapsed ? <ChevronRight size={20}/> : <ChevronLeft size={20}/>}
                {!collapsed && <span className="font-bold text-xs uppercase">Recolher</span>}
             </div>
           </button>

           <button onClick={handleLogout} className="p-3 w-full flex items-center gap-3 text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all">
             <div className="w-full flex justify-center items-center gap-3">
                <LogOut size={20}/>
                {!collapsed && <span className="font-bold text-xs uppercase">Sair</span>}
             </div>
           </button>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-hidden p-4">
        {isLoading ? (
          <div className="h-full flex items-center justify-center flex-col gap-4 text-blue-600">
             ...
             <p className="font-bold animate-pulse">Carregando dados com segurança...</p>
          </div>
        ) : (
          <Routes>
            <Route path="/" element={
